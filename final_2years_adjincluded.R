...
############# Assigning filtered values to dat_coal and dat_gas
...
####### COAL- Creating DF  based of 24h for every day of 2018 and 2017
...
# daily df_result (df_result) (drop Na caused by our codes ( not data ))
...
#You  can switch here,adjsut df_cut or not_cut
df_mc_coal<-df_cut_coal



df_mc_coal<-df_mc_coal%>% 
  select(Company,PP,month,day,MC,Efficiency,Fuel,`Coal price`,`Gas price`,EUA,year)  %>%
  arrange(Company,PP,year,month,day)

#select only one value per a day 
df_mc_coal<-df_mc_coal%>% dplyr:: select(-MC)
df_mc_coal<-unique(df_mc_coal)

df_mc_coal_day<- merge(df_day_coal,df_mc_coal,by=c("Company","PP","year","month","day"),all.x=TRUE)

df_mc_coal_day[,7:30]<-NULL
df_mc_coal_day$Fuel<-NULL
df_mc_coal_day$`Gas price`<-NULL

####

## FUNCTION
f_mc_coal_dfcreate<-function(row,day){
  y<- data.frame(matrix(ncol = day*3,nrow = row))
  x<- list()
  for (i in 1:(day))
  {x<-append(x,paste(c("coalp","EUA","Adj_MC"),i, sep = "_") )}
  colnames(y)<- x
  y
}
#######################################
##empty dataframe created 
df_adj<-f_mc_coal_dfcreate(nrow(df_mc_coal_day),day)
df_mr_adj<-cbind(df_mc_coal_day,df_adj)

#### Create index column for each pp of a Company
df_mc_idex<- data.frame(matrix(ncol = ncol(df_mr_adj),nrow = 1))
colnames(df_mc_idex)<- colnames(df_mr_adj)
df_mc_idex$index<- NA

for ( c in as.character(unique(df_mr_adj$Company)) ){
  f_df1<-df_mr_adj %>%filter(Company==c)
  
  for ( p in as.character(unique(f_df1$PP)) ){
    f_df2<-f_df1 %>%filter(PP==p) %>% arrange(year,month,day)
    f_df2$index<-1:nrow(f_df2)
    df_mc_idex<- rbind(df_mc_idex,f_df2)
    
  }
}

#drop (first row na )
df_mc_idex<- df_mc_idex[-1,]

#### Assign values to df_mc_index

df_adj_coal<- data.frame(matrix(ncol = ncol(df_mc_idex),nrow = 1))
colnames(df_adj_coal)<- colnames(df_mc_idex)

for ( c in as.character(unique(df_mc_idex$Company)) ){
  f_df1<-df_mc_idex %>%filter(Company==c)
  ...
  for ( p in as.character(unique(f_df1$PP)) ){
    ...
    for ( i in day:730){
      for (r in 1:(day)){
        ...
        f_df2[f_df2$index==i+1,3*(r+2)+1]<-f_df2[f_df2$index==i+1,8]- f_df2[f_df2$index==i+1-r,8]
        f_df2[f_df2$index==i+1,3*(r+2)+2]<-f_df2[f_df2$index==i+1,9]- f_df2[f_df2$index==i+1-r,9]  
        f_df2[f_df2$index==i+1,3*(r+2)+3]<- (f_df2[f_df2$index==i+1,3*(r+2)+1]+ f_df2[f_df2$index==i+1,3*(r+2)+2])/f_df2[f_df2$index==i+1,]$Efficiency
      }
    }
    
    df_adj_coal<-rbind(df_adj_coal,f_df2)
  }
}
...
#drop first row because of Na caused by codes
df_adj_coal <- df_adj_coal[-1,]

#drop rows smaller than day(window size 90) 
# DROP INDEX COLUMN
# we dont intrested day smaller than  WINDOW SIZE 90. Therefore, it starts 01.04.2017

df_adj_coal <- df_adj_coal[df_adj_coal$index>day,]
df_adj_coal$index<-NULL

#select only adjust_delta_mc (Adj_MC) columns

df_adj_mc_coal<- cbind(df_adj_coal[,1:5],data.frame(df_adj_coal[,grepl("Adj_MC",names(df_adj_coal))]))

###CREATING dataframe for df_Company_coal to transform df_adj_mc_coal for adding df_day_coal 
######### using same df
df_adj_coal_trasform <- df_Company_coal

ncol(df_adj_mc_coal)
ncol(df_adj_coal_trasform)
nrow(df_adj_mc_coal)
nrow(df_adj_mc_coal)
...
for( x in 1:day){
  df_adj_coal_trasform[,(7+24*x): (6+24*(x+1))]<-df_adj_mc_coal[,5+x]
}

#Assgign zero to na values  and today bids
df_adj_coal_trasform[,7:30] <- 0
df_adj_coal_trasform[is.na(df_adj_coal_trasform)] <- 0

####summation of two dataframe ( ) 
nrow(df_adj_coal_trasform)
nrow(df_Company_coal)
ncol(df_adj_coal_trasform)
ncol(df_Company_coal)

df1_coal<-df_Company_coal



# df2 zero assigned to today values ( no need to adjust to today Mc values )
df2_coal<-df_adj_coal_trasform
df3_coal<-cbind(df1_coal[1:6], df1_coal[-(1:6)]+df2_coal[-(1:6)])


#############################
#############################  the adjustment according to today MC done !!!!!!!
#############################
#############################

# First, I stored not-adjusted version. It can be switched off adjested to not-adjested 
#by this codes 

not_adjusted_df_Company_coal<-df_Company_coal
#df_Company_coal <-df3_coal
...
##########  SAME STEPS FOR GAS POWER PLANTS
##########
##########
########### GAS
###########
...
#select only one value per a day 
df_mc_gas<-df_mc_gas%>% dplyr:: select(-MC)
df_mc_gas<-unique(df_mc_gas)

df_mc_gas_day<- merge(df_day_gas,df_mc_gas,by=c("Company","PP","year","month","day"),all.x=TRUE)

df_mc_gas_day[,7:30]<-NULL
df_mc_gas_day$Fuel<-NULL
df_mc_gas_day$`Coal price`<-NULL

####

## FUNCTION
f_mc_gas_dfcreate<-function(row,day){
  y<- data.frame(matrix(ncol = day*3,nrow = row))
  x<- list()
  for (i in 1:(day))
  {x<-append(x,paste(c("gasp","EUA","Adj_MC"),i, sep = "_") )}
  colnames(y)<- x
  y
}
#######################################
##empty dataframe created 
df_adj<-f_mc_gas_dfcreate(nrow(df_mc_gas_day),day)
df_mr_adj<-cbind(df_mc_gas_day,df_adj)
...
df_adj_gas<- data.frame(matrix(ncol = ncol(df_mc_idex),nrow = 1))
colnames(df_adj_gas)<- colnames(df_mc_idex)
...
for ( c in as.character(unique(df_mc_idex$Company)) ){
  ...
  for ( p in as.character(unique(f_df1$PP)) ){
    ...
    df_adj_gas<-rbind(df_adj_gas,f_df2)
  }
}

copy_df_adj_gas <- df_adj_gas
#df_adj_gas<-copy_df_adj_gas 

#drop first row because of Na caused by codes
df_adj_gas <- df_adj_gas[-1,]

#drop rows smaller than day(window)  value and DROP INDEX COLUMN
# we dont intrested day smaller than  WINDOW SIZE 

df_adj_gas <- df_adj_gas[df_adj_gas$index>day,]
df_adj_gas$index<-NULL

#select only adjust_delta_mc (Adj_MC) columns

df_adj_mc_gas<- cbind(df_adj_gas[,1:5],data.frame(df_adj_gas[,grepl("Adj_MC",names(df_adj_gas))]))

###CREATING dataframe for df_Company_gas to transform df_adj_mc_gasfor adding df_day_gas 
######### using same df
df_adj_gas_trasform <- df_Company_gas

ncol(df_adj_mc_gas)
ncol(df_adj_gas_trasform)



for( x in 1:day){
  df_adj_gas_trasform[,(7+24*x): (6+24*(x+1))]<-df_adj_mc_gas[,5+x]
}

#Assgign zero to na values 
df_adj_gas_trasform[,7:30] <- 0
df_adj_gas_trasform[is.na(df_adj_gas_trasform)] <- 0

####summation of two dataframe ( ) 
nrow(df_adj_gas_trasform)
nrow(df_Company_gas)
ncol(df_adj_gas_trasform)
ncol(df_Company_gas)

df1_gas<-df_Company_gas



# df2 zero assigned to today values ( no need to adjust to today Mc values )
df2_gas<-df_adj_gas_trasform
df3_gas<-cbind(df1_gas[1:6], df1_gas[-(1:6)]+df2_gas[-(1:6)])
...
not_adjusted_df_Company_gas<-df_Company_gas
#df_Company_gas <-df3_gas
